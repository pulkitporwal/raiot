...
const Members = () => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      setIsLoading(true)
      const response = await fetch("https://raiot-1.onrender.com/api/user/fetch-members", { credentials: "include" })
  ...
  const handleSubmit = async () => {
    try {
      setIsLoading(true)
      const response = await fetch("https://raiot-1.onrender.com/api/user/update-members", {
        method: "POST",
        headers: {
        ...
        <div className="overflow-x-auto mt-10 pb-3">
          <table className="table-auto min-w-max">
            ...
            <tbody>
              {
                members.map((member) => (
                  <tr key={member._id} data-key={member._id} className={`${currentUser && member._id === currentUser._id ? "bg-orange-800" : ""}`}>
                    ...
                    <td className="text-center border-b-[1px] border-[#565656] mx-10 px-4 py-2">
                      <select className="bg-[#322e74]" onChange={handleChange} name="userType" id="userType" value={member.userType}>
                    ...
                    <td className="min-w-32 text-center border-b-[1px] border-[#565656] mx-10 px-4 py-2">
                      <select className="bg-[#322e74]" onChange={handleChange} name="banUserToIssueComponent" id="banUserToIssueComponent" value={member.banUserToIssueComponent.toString()}>
