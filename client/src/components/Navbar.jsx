...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    ...
    const response = await fetch("https://raiot-1.onrender.com/api/auth/signout", {
      method: "POST",
      headers: {
  ...
  return (
    <>
      <div className="fixed top-0 left-0 w-full bg-[#11101d] p-4 flex justify-between items-center sm:hidden z-50">
        <Link to={"/dashboard"}>
          <div className="flex items-center gap-2">
            <img src={logo} className="w-8 bg-white rounded-full" alt="logo" />
            <h2 className="text-white text-xl font-extrabold font-[Poppins]">
              RAIoT
            </h2>
          </div>
        </Link>
      ...
      <div className={`fixed top-0 left-0 w-full sm:w-[250px] h-screen bg-[#11101d] transition-transform duration-300 ease-in-out transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} sm:translate-x-0 flex flex-col justify-between items-center px-6 py-5 z-40 overflow-y-auto`}>
        {loading && <Loader />}
        ...
        <ul className="flex list-none w-full items-start justify-center flex-col gap-y-4 mt-16 sm:mt-0">
          {displayClubMember && (
            <li
              onClick={() => {
        ...
        <div
          ...
          className="group relative rounded-lg bottom-0 w-[100%] right-0 flex items-center justify-between gap-5 cursor-pointer hover:bg-[#ebdc36] py-2 px-5 mt-4"
        >
